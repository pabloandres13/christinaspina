		...
		if (class_exists('YPRM_Novo_Addons') && $attachment_ids = $product->get_gallery_image_ids()) {
			if (is_array($attachment_ids)) {
				...
				foreach ($attachment_ids as $img_item) {
					$output .= '<div class="swiper-slide"><a href="' . get_the_permalink() . '" class="img" style="'.yprm_get_image($img_item, 'bg', 'large').'"></a></div>';
				}
				$output .= '</div>';
			...
			$output .= '<a href="' . get_the_permalink() . '" class="img" style="'.yprm_get_image($product->get_image_id(), 'bg', 'large').'"></a>';
